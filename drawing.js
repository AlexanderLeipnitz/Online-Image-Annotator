...
function draw_Annotation(bboxes, polygons) {
  ...
  if (annotation_type == "detection") {
    ...
    for (var i = 0; i < detection_classes.length; i++) {
      document.getElementById("".concat(detection_classes[i][0], "_num")).innerHTML = nums[i];
    }
    ...
    for (var i = 0; i < polygons.length; i++) {
      ...
      for (var k = 0; k < 2; k++) {
        ...
        for (var j = 0; j < current_polygon.x_coords.length; j++) {
          ...
          let x_start = (((Number(current_polygon.x_coords[j]) - offset_x) / scale_factor_orig)) * scale;
          let y_start = (((Number(current_polygon.y_coords[j]) - offset_y) / scale_factor_orig)) * scale;
          main_ctx.moveTo(x_start, y_start);
          var x_end = (((Number(current_polygon.x_coords[j + 1]) - offset_x) / scale_factor_orig)) * scale;
          var y_end = (((Number(current_polygon.y_coords[j + 1]) - offset_y) / scale_factor_orig)) * scale;
          if ((j + 1) >= current_polygon.x_coords.length) {
            x_end = (((Number(current_polygon.x_coords[0]) - offset_x) / scale_factor_orig)) * scale;
            y_end = (((Number(current_polygon.y_coords[0]) - offset_y) / scale_factor_orig)) * scale;
          }
    ...
    for (var i = 0; i < segmentation_classes.length; i++) {
      document.getElementById("".concat(segmentation_classes[i][0], "_num")).innerHTML = nums[i];
    }
  }
}
...
function draw_Image(img_name, offset_x, offset_y, update_preview) {
  ...
  img.onload = function() {
    ...
    if (scale_factor != 4) { zoomed_in = true; } else { zoomed_in = false; }
  ...
  load_img_ajax("img_load.php?img_name=" + img_name + "&offset_x=" + offset_x +
    "&offset_y=" + offset_y + "&canvas_w=" + main_canvas.width +
    "&canvas_h=" + main_canvas.height + "&scale_factor=" + scale_factor).then(function(response) {
...
function zoom_img(zoom_diff) {
  ...
  if (scale_factor_new >= 4 && scale_factor_new <= 128) {
    ...
    if (offset_x + (width_new) > width_orig) { offset_x = width_orig - width_new; }
    if (offset_y + (height_new) > height_orig) { offset_y = height_orig - height_new; }
    if (scale_factor_new == 4) { offset_x = 0; }
    if (scale_factor_new == 4) { offset_y = 0; }
...
function move_img(direction) {
  if (scale_factor > 4) {
    var scale = scale_factor / scale_factor_orig;
    if (direction == '87') { offset_y -= zoom_stride / scale; } // w
    else if (direction == '65') { offset_x -= zoom_stride / scale; } // a
    else if (direction == '83') { offset_y += zoom_stride / scale; } // s
    else if (direction == '68') { offset_x += zoom_stride / scale; } // d

    if (offset_x < 0) { offset_x = 0; }
    if (offset_y < 0) { offset_y = 0; }
    if (offset_x + (width_orig / scale) > width_orig) { offset_x = width_orig - width_orig / scale; }
    if (offset_y + (height_orig / scale) > height_orig) { offset_y = height_orig - height_orig / scale; }
