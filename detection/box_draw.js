...
function finalized_drawn_rect(e) {
  ...
  if (pos_start.x != pos.x && pos_start.y != pos.y) {
    ...
    var left_offset = 1;
    var top_offset = 1;
    if (pos_start.x == transparent_canvas.width - 1) { right_offset = 0; }
    if (pos_start.y == transparent_canvas.height - 1) { bottom_offset = 0 }
    if (pos.x == 0) { left_offset = 0 }
    if (pos.y == 0) { top_offset = 0 }
    let xmin = (pos_start.x + right_offset) / scale * scale_factor_orig + offset_x;
    let ymin = (pos_start.y + bottom_offset) / scale * scale_factor_orig + offset_y;
    ...
    if (!bbox_list.includes([xmin, ymin, xmax, ymax, detection_classes[current_class_id][0]])) {
      bbox_list.push([xmin, ymin, xmax, ymax, detection_classes[current_class_id][0]]);
    }
...
function animate_rect_drawing(e) {
  ...
  setPosition(e);
  transparent_ctx.rect(pos_start.x, pos_start.y, pos.x - pos_start.x, pos.y - pos_start.y);
  transparent_ctx.stroke(); // draw it!
  transparent_ctx.closePath();
}