...
for i in range(len(classes)):
    stats_per_class.append({'num_of_boxes': 0, 'min_size_abs': 999999, 'max_size_abs': 0, 'summed_size_abs': 0})
...
for i in tqdm(range(len(dirlist))):
    ...
    for j in range(len(vidlist)):
        inputdir = os.path.join(finished_annotations, os.path.join(dirlist[i], vidlist[j]))
...
for i in tqdm(range(len(dirlist))):
    ...
    for j in range(len(vidlist)):
        inputdir = os.path.join(unfinished_annotations, os.path.join(dirlist[i], vidlist[j]))
        filelist = os.listdir(inputdir)
        #print('filelist: ', filelist)
...
for i in range(len(classes)):
    file.write('{}_num_of_boxes,{}\n'.format(classes[i], stats_per_class[i]['num_of_boxes']))
    file.write('{}_min_size_abs,{}\n'.format(classes[i], stats_per_class[i]['min_size_abs']))
    file.write('{}_max_size_abs,{}\n'.format(classes[i], stats_per_class[i]['max_size_abs']))
    file.write('{}_summed_size_abs,{}\n'.format(classes[i], stats_per_class[i]['summed_size_abs']))
