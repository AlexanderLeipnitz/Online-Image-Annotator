""" ########################################################################
 ''
 '' File....:	move_finished_annotations.py
 '' Function:	Move images and annotations with the tag "finished" from the
 ''             "unfinished" folder to the "finished" folder
...
def main():
    ...
    base_path_output_xml = "../data/finished/annotations/"
    base_path_input_img = "../data/unfinished/imgs/"
    base_path_output_img = "../data/finished/imgs/"
    base_path_input_img_small = "../data/unfinished/imgs_small/"
    base_path_output_img_small = "../data/finished/imgs_small/"
    ...
    for i in range(len(dirlist)):
        ...
        for j in range(len(vidlist)):
            # Build paths
            inputdir_xml = os.path.join(base_path_input_xml, dirlist[i], vidlist[j])
            outputdir_xml = os.path.join(base_path_output_xml, dirlist[i], vidlist[j])
            inputdir_img = os.path.join(base_path_input_img, dirlist[i], vidlist[j])
            outputdir_img = os.path.join(base_path_output_img, dirlist[i], vidlist[j])
            inputdir_img_small = os.path.join(
                base_path_input_img_small, dirlist[i], vidlist[j]
            )
            outputdir_img_small = os.path.join(
                base_path_output_img_small, dirlist[i], vidlist[j]
            )
                ...
                if os.path.splitext(filelist[file_idx])[1] == ".xml":
                    ...
                    for xml_object in root.findall("status"):  # check tags
                        if xml_object.text == "finished":
                            ...
                            os.rename(
                                xml_file,
                                os.path.join(outputdir_xml, filelist[file_idx]),
                            )
                            img_name = filelist[file_idx].replace(".xml", ".jpg")
                            os.rename(
                                os.path.join(inputdir_img, img_name),
                                os.path.join(outputdir_img, img_name),
                            )
                            os.rename(
                                os.path.join(inputdir_img_small, img_name),
                                os.path.join(outputdir_img_small, img_name),
                            )
