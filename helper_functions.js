...
function mouse_inside_bbox(e, margin, tool_mode) {
  ...
  for (var i = 0; i < bbox_list.length; i++) {
    ...
    if (x >= xmin + margin && x <= xmax - margin && y >= ymin + margin && y <= ymax - margin) {
      current_bbox = i;
...
function check_mouse_on_line(e) {
  ...
  for (var i = 0; i < bbox_list.length; i++) {
    ...
    var xmax = (((Number(object[2]) - offset_x) / scale_factor_orig)) * scale;
    var ymax = (((Number(object[3]) - offset_y) / scale_factor_orig)) * scale;
    if (x >= xmin - margin && x <= xmax + margin && y >= ymin - margin && y <= ymin + margin) { //top
      current_bbox = i;
      top = true;
    }
    if (x >= xmin - margin && x <= xmin + margin && y >= ymin - margin && y <= ymax + margin) { //left
      current_bbox = i;
      left = true;
    }
    if (x >= xmax - margin && x <= xmax + margin && y >= ymin - margin && y <= ymax + margin) { //right
      current_bbox = i;
      right = true;
    }
    if (x >= xmin - margin && x <= xmax + margin && y >= ymax - margin && y <= ymax + margin) { //bottom
      current_bbox = i;
      bottom = true;
    }
...
function checkKey(e) {
  ...
  if (e.keyCode == 70) { //key: f
    ...
    zoom_img(-2);
  } else if (e.keyCode >= 49 && e.keyCode <= 51 && annotation_type == "detection") { //key: 1 to 3
