...
function animate_polygon_drawing(polygon) {
  ...
  for (var k = 0; k < 2; k++) {
    ...
    for (var i = 1; i < polygon.x_coords.length; i++) {
      ...
      var scale = scale_factor / scale_factor_orig;
      let x_start =
        ((polygon.x_coords[i - 1] - offset_x) / scale_factor_orig) * scale;
      let y_start =
        ((polygon.y_coords[i - 1] - offset_y) / scale_factor_orig) * scale;
      let x_end =
        ((polygon.x_coords[i] - offset_x) / scale_factor_orig) * scale;
      let y_end =
        ((polygon.y_coords[i] - offset_y) / scale_factor_orig) * scale;
      console.log(x_start);
  ...
  if (polygon.x_coords.length > 2) {
    var x_start =
      ((polygon.x_coords[0] - offset_x) / scale_factor_orig) * scale;
    var y_start =
      ((polygon.y_coords[0] - offset_y) / scale_factor_orig) * scale;
  ...
  console.log(end.x);
  let x_prev =
    ((polygon.x_coords[polygon.x_coords.length - 1] - offset_x) /
      scale_factor_orig) *
    scale;
  let y_prev =
    ((polygon.y_coords[polygon.y_coords.length - 1] - offset_y) /
      scale_factor_orig) *
    scale;
