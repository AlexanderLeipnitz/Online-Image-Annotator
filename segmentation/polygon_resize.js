...
function check_mouse_poly_distance(e, tool_mode) {
  ...
  for (var i = 0; i < polygon_list.length; i++) {
    ...
    for (var j = 0; j < current_polygon.x_coords.length; j++) {
      let x =
        ((Number(current_polygon.x_coords[j]) - offset_x) / scale_factor_orig) *
        scale;
      let y =
        ((Number(current_polygon.y_coords[j]) - offset_y) / scale_factor_orig) *
        scale;
  ...
  if (highlight && polygon_idx > -1 && point_idx > -1) {
    ...
    for (var idx = start_idx; idx <= end_idx; idx++) {
      let x =
        ((polygon_list[polygon_idx].x_coords[idx] - offset_x) /
          scale_factor_orig) *
        scale;
      let y =
        ((polygon_list[polygon_idx].y_coords[idx] - offset_y) /
          scale_factor_orig) *
        scale;
      transparent_ctx.beginPath(); // begin
...
function animate_polgon_resizing(e, point_idx) {
  ...
  for (var j = 0; j < modified_polygon.x_coords.length; j++) {
    ...
    if (j == point_idx) {
      ...
      x_start =
        ((Number(modified_polygon.x_coords[j]) - offset_x) /
          scale_factor_orig) *
        scale;
      y_start =
        ((Number(modified_polygon.y_coords[j]) - offset_y) /
          scale_factor_orig) *
        scale;
    }
    ...
    if (j == 0 && point_idx == 0) {
      x_end =
        ((Number(
          modified_polygon.x_coords[modified_polygon.x_coords.length - 1]
        ) -
          offset_x) /
          scale_factor_orig) *
        scale;
      y_end =
        ((Number(
          modified_polygon.y_coords[modified_polygon.x_coords.length - 1]
        ) -
          offset_y) /
          scale_factor_orig) *
        scale;
      ...
      x_end =
        ((Number(modified_polygon.x_coords[0]) - offset_x) /
          scale_factor_orig) *
        scale;
      y_end =
        ((Number(modified_polygon.y_coords[0]) - offset_y) /
          scale_factor_orig) *
        scale;
    } else {
      x_end =
        ((Number(modified_polygon.x_coords[j + 1]) - offset_x) /
          scale_factor_orig) *
        scale;
      y_end =
        ((Number(modified_polygon.y_coords[j + 1]) - offset_y) /
          scale_factor_orig) *
        scale;
    }
...
function finalize_resized_polygon(e, current_point_idx) {
  ...
  polygon_list[current_polygon_idx].x_coords[current_point_idx] =
    (pos.x / scale) * scale_factor_orig + offset_x;
  polygon_list[current_polygon_idx].y_coords[current_point_idx] =
    (pos.y / scale) * scale_factor_orig + offset_y;
